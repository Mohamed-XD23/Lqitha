        ...
        <div className="mb-8 flex items-center justify-between">
            {steps.map((label, index) => {
                ...
                ${isCompleted ? "bg-black text-white" : isCurrent ? "border-2 border-black text-black" : "border-2 border-gray-300 text-gray-400"}`}>
                            ...
                            <span className={`mt-1 text-xs ${isCurrent ? "font-semibold text-black" : "text-gray-400"}`}>
                                {label}
                            </span>
                        </div>
                        {index < steps.length - 1 && (
                            <div className={`mx-2 mb-4 h-px flex-1 ${isCompleted ? "bg-black" : "bg-gray-300"}`} />
                        )}
                    </div>
                );
            })}
...
export default function NewItemPage() {
    ...
    async function handleNext() {
        ...
        if (step === 1) fieldsToValidate = ["type", "title", "category", "description"];
        if (step === 2) fieldsToValidate = ["location", "date", "phone"];
        if (step === 3 && watchedType === "FOUND") fieldsToValidate = ["secretQuestion", "secretAnswer"];
    ...
    return (
        <div className="mx-auto max-w-xl px-4 py-10">
            ...
            <p className="mb-8 text-sm text-gray-500">أدخل تفاصيل الغرض لمساعدة الآخرين في إيجاده</p>
            ...
            {step === 1 && (
                <div className="flex flex-col gap-5">
                    <div>
                        <label className="mb-2 block text-sm font-semibold">نوع البلاغ</label>
                        <div className="grid grid-cols-2 gap-3">
                            {(["LOST", "FOUND"] as const).map((t) => (
                                <label key={t} className={`flex cursor-pointer flex-col items-center rounded-xl border-2 p-4 transition-colors
                  ${watchedType === t ? "border-black bg-black text-white" : "border-gray-200 hover:border-gray-400"}`}>
                                    ...
                                    <span className="mt-1 font-semibold">{t === "LOST" ? "مفقود" : "موجود"}</span>
                    ...
                    <div>
                        ...
                        <input {...register("title")} placeholder="مثال: مفتاح سيارة تويوتا"
                    ...
                    <div>
                        ...
                        <select {...register("category")}
                            ...
                            ["DOCUMENTS", "وثائق"], ["ELECTRONICS", "إلكترونيات"], ["OTHER", "أخرى"]].map(([v, l]) => (
            ...
            {step === 2 && (
                <div className="flex flex-col gap-5">
                    <div>
                        ...
                        <input {...register("location")} placeholder="مثال: الجزائر العاصمة"
                            className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black" />
                        {errors.location && <p className="mt-1 text-xs text-red-500">{errors.location.message}</p>}
                    </div>

                    <div>
                        <label className="mb-1 block text-sm font-semibold">التاريخ والوقت</label>
                        <input
                            {...register("date")}
                            type="datetime-local"
                    ...
                    <div>
                        ...
                        <input {...register("phone")} placeholder="05XXXXXXXX" type="tel"
                            className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black" />
                    ...
                    <div>
                        ...
                        <input {...register("imageUrl")} placeholder="رابط الصورة"
                            className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black" />
                    </div>
                </div>
            )}
            ...
            {step === 4 && (
                <div className="flex flex-col gap-4">
                    {[
                        { label: "نوع البلاغ", value: watchedValues.type === "LOST" ? "🔍 مفقود" : "📦 موجود" },
                        ...
                        <div key={label} className="flex justify-between rounded-lg border px-4 py-3 text-sm">
